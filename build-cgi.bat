...
%SL_AGENT% config --TokenFile "%cd%\\sealights.token" --appName %SL_APP_NAME% --branchName %SL_BRANCH_NAME% --buildName %SL_BUILD_NAME% --includeNamespace CGI.Ratabase.WebApi.Common.*,CGI.Ratabase.WebApi.Distribution.*,CGI.Ratabase.WebAPI.Reports.*,CGI.Ratabase.WebApi.PTR.*,CGI.Ratabase.WebApi.FormulaCompiler.* --proxy %SL_PROXY% --buildSessionIdFile buildSession --logEnabled true --ignoreCertificateErrors true
pause
